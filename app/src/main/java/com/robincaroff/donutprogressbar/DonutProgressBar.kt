...
open class DonutProgressBar @JvmOverloads constructor(
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        arcOval.set(center.x - arcRadius, center.y - arcRadius, center.x + arcRadius, center.y + arcRadius)
        invalidate()
    }
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        canvas.drawCircle(center.x.toFloat(), center.y.toFloat(), backgroundCircleRadius, backgroundCirclePaint)

        canvas.drawArc(arcOval, -90f, progressEndAngle, false, arcPaint)

        canvas.drawText(text, center.x.toFloat() - textPaint.measureText(text) / 2f, center.y.toFloat() - ((textPaint.descent() + textPaint.ascent()) / 2), textPaint)
    }
}